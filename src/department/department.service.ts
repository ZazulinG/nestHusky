...
import {Employee, EmployeeDocument} from "../employee/entities/employee.entity";
...
export class DepartmentService {
  ...
  async create(createDepartmentDto: CreateDepartmentDto): Promise<Department> {
    const department = await this.departmentModel.findOne({name: createDepartmentDto.name})
    if(department){
  ...
  async update(id: string, updateDepartmentDto: UpdateDepartmentDto): Promise<Department>  {
    ...
    const department = await this.departmentModel.findOne({_id: id})
    if(department){
      return this.departmentModel.findOneAndUpdate({_id: id}, updateDepartmentDto, {new: true});
    }
    throw new NotFoundException()
  }
  ...
  async deleteEmployeeFromDepartment(idEmp: string, idDep: string){
    validate(idDep)
    validate(idEmp)
    const emp = await this.employeeModel.findOne({_id: idEmp, department: idDep})
    if(emp){
      await this.employeeModel.updateOne({_id: idEmp}, {$set: {department: null}})
